...
from multijudge.internvl import (  # noqa: E402
    IMAGE_SIZE,
    MAX_TILES_PER_IMAGE,
    MAX_TOTAL_TILES,
    InternVLJudgeAdapter,
)
from multijudge.minicpm import MAX_SLICE_NUMS, MiniCPMJudgeAdapter  # noqa: E402
from multijudge.policy import load_policy  # noqa: E402
from multijudge.runtime import collect_environment  # noqa: E402
from multijudge.safework import (  # noqa: E402
    MAX_PIXELS,
    MIN_PIXELS,
    SafeWorkJudgeAdapter,
)
...
def build_adapter(args: argparse.Namespace, policy: Any) -> Any:
    common = {
    ...
    if args.adapter == "safework":
        return SafeWorkJudgeAdapter(
            **common,
            attn_implementation=args.attn_implementation,
        )
    if args.adapter == "internvl":
        return InternVLJudgeAdapter(
            **common,
            attn_implementation=args.attn_implementation,
        )
    if args.adapter == "minicpm":
        return MiniCPMJudgeAdapter(
            **common,
            attn_implementation=args.attn_implementation,
        )
...
def adapter_settings(adapter: str) -> dict[str, Any]:
    if adapter == "safework":
    ...
    if adapter == "minicpm":
        return {"max_slice_nums": MAX_SLICE_NUMS}
...
def run(args: argparse.Namespace) -> None:
    ...
    adapter = build_adapter(args, policy)

    model_fingerprint = json.loads(
        args.model_fingerprint.read_text(encoding="utf-8")
    )
    if Path(model_fingerprint["model_path"]).resolve() != args.model_path.resolve():
        raise RuntimeError(
    ...
    config = {
        ...
        "model_revision": args.model_revision,
        "model_manifest_sha256": model_fingerprint["manifest_sha256"],
        "seed": args.seed,
        "max_new_tokens": args.max_new_tokens,
        "attn_implementation": args.attn_implementation,
        "adapter_settings": adapter_settings(args.adapter),
    ...
    try:
        with result_path.open("a", encoding="utf-8", buffering=1) as output:
            for index, sample in enumerate(progress_iterator(pending), start=1):
                record = adapter.judge(
                    sample,
                    input_mode="native",
                    panel_dir=run_dir / "unused-panels",
                )
                output.write(
        ...
        if final_state != "completed":
            raise RuntimeError(
                f"Qualification gate failed: {parser_errors}/{len(samples)} "
                "parser errors"
            )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run a resumable native multi-image judge qualification."
    )
    parser.add_argument(
        "--adapter",
        choices=("safework", "internvl", "minicpm"),
        required=True,
    )
    ...
    parser.add_argument("--model-fingerprint", type=Path, required=True)
